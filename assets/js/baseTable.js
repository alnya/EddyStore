...
define(['knockout', 'common', 'webApiClient','moment'], function(ko, common, webApiClient, moment) {
    ...
    var gridViewModel = function(config) {
        ...
        var getSearchParams = function () {
            var params = "Page=" + (self.pageIndex() + 1) + "&Pagesize=" + self.pageSize();
            var sortColumn = self.sortColumn();

            if ((sortColumn != null && sortColumn.sortDescending() != null)) {
                params += "&SortBy=" + sortColumn.sortValue + "&SortDescending=" + sortColumn.sortDescending();
            }
                    ...
                    if (term.dataType == "DateRange") {
                        ...
                        if (dates.length == 2) {
                            ...
                            if (dateFrom.isValid() && dateTo.isValid()) {
                                params += "&" + term.fieldName + ":from=" + dateFrom.format('YYYY-MM-DD');
                                params += "&" + term.fieldName + ":to=" + dateTo.format('YYYY-MM-DD');
                            }
                        } else {
                            params += "&" + term.fieldName + "=" + term.filterValue;
                        }
                    } else {
                        params += "&" + term.fieldName + "=" + term.filterValue;
                    }
